			    ...
			    struct tun_file *tfile)
{
	...
	if (likely(e)) {
		...
		printk("tun_flow_update, find e, e->rxhash:%u, e->queue_index:%d, tfile->queue_index:%u\n", e->rxhash, e->queue_index, tfile->queue_index);
		...
		spin_unlock_bh(&tun->lock);
		printk("tun_flow_update, create e, rxhash:%u, e->queue_index=tfile->queue_index:%u\n", rxhash, tfile->queue_index);
	}
...
static u16 tun_automq_select_queue(struct tun_struct *tun, struct sk_buff *skb)
{
	...
	printk("tun_automq_select_queue, rxhash:%u, skb->len:%u, skb->queue_mapping:%u \n", txq, skb->len, skb->queue_mapping);
	e = tun_flow_find(&tun->flows[tun_hashfn(txq)], txq);
	if (e) {
		tun_flow_save_rps_rxhash(e, txq);
		txq = e->queue_index;
		printk("tun_automq_select_queue, fine e, e->queue_index:%u, skb->len:%u \n", e->queue_index, skb->len);
	} else {
		/* use multiply and shift instead of expensive divide */
		txq = ((u64)txq * numqueues) >> 32;
	}
	printk("final, tun_automq_select_queue, txq:%u, skb->len:%u \n", txq, skb->len);
	return txq;
}
			   ...
			   struct sk_buff *skb, int more)
{
	...
	printk("rx_batched:%u, more:%d, skb_queue_empty(queue)):%d\n", rx_batched, more, skb_queue_empty(queue));
...
drop:
	...
	if (rxhash){
		printk("111111111, tfile->queue_index:%d, rxhash:%u, skb->len:%u, frags:%d, napi_enabled:%d\n",tfile->queue_index, rxhash, skb->len, frags,tfile->napi_enabled);
	...
	if (frags) {
		...
		if (rxhash){
			printk("2222 frags, befor tun_flow_update, rxhash:%u, skb->len:%u", rxhash, skb->len);
			tun_flow_update(tun, rxhash, tfile);
		}
		...
		int queue_len;
		if (rxhash){
			printk("33333333 tfile->napi_enabled, befor tun_flow_update, rxhash:%u, skb->len:%u", rxhash, skb->len);
			tun_flow_update(tun, rxhash, tfile);
		}
		...
		printk("444444444444444444444444, rxhash:%u, skb->len:%u\n", rxhash, skb->len);
		tun_rx_batched(tun, tfile, skb, more);
	} else {
				if (rxhash){
			printk("555555555555 befor tun_flow_update, rxhash:%u, skb->len:%u", rxhash, skb->len);
			tun_flow_update(tun, rxhash, tfile);
		}
		netif_rx_ni(skb);
	}
	...
	put_cpu_ptr(stats);
	if (rxhash){
		printk("last, befor tun_flow_update, rxhash:%u, skb->len:%u", rxhash, skb->len);
		tun_flow_update(tun, rxhash, tfile);
	}
	return total_len;
}
